...
const Sidebar = () => {
  ...
  return (
    ...
    >
      <img
        src={control}
        className={`absolute cursor-pointer -right-3 top-9 w-7 border-dark_color border-2 rounded-full ${
          !open && "rotate-180"
        }`}
        ...
        <Link to="/logout" onClick={logout}>
          ...
          >
            <div className="flex items-center">
              ...
              />
              <span
                className={`ml-5 ${!open && "hidden"} origin-left duration-200`}
              >
                Logout
              </span>
            </div>
      ...
      <div className="mt-5">
        <span className={`text-white ml-9 mt-10  ${!open && "hidden"}`}>
          © All rights reserved by
        </span>
        <div className="flex flex-col gap-4 items-center">
          <img
            src={xyma}
            className={`cursor-pointer w-40 mt-2 duration-500 `}
          />
        </div>
      </div>
    </div>
