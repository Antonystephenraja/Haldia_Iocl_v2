...
const PopupComponent = ({ onClose }) => {
  ...
  const handleDownload = async () => {
    ...
    try{
      const response = await fetch('http://localhost:4000/backend/Report_data', {
        method: 'POST',
        headers: {
...
const Demo = ({Device_Limit,Alldata}) => {
  ...
  const handleDaysDropdown =async (selectedOption) => {  
      ...
      // await axios.post('http://localhost:4000/backend/saveData', { inputValue: selectedOption.value });
